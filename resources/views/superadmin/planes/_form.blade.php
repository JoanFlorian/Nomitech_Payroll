...
<div x-data="{
    ...
    features: [
        '{{ old('features.0', (isset($plan->features) && is_array($plan->features)) ? ($plan->features[0] ?? '') : '') }}',
        '{{ old('features.1', (isset($plan->features) && is_array($plan->features)) ? ($plan->features[1] ?? '') : '') }}',
        '{{ old('features.2', (isset($plan->features) && is_array($plan->features)) ? ($plan->features[2] ?? '') : '') }}',
        '{{ old('features.3', (isset($plan->features) && is_array($plan->features)) ? ($plan->features[3] ?? '') : '') }}'
    ],
    touched: {
        nombre: false,
        duracion: false,
    ...
    validate() {
        ...
        // Nombre
        if (!nombreTrim) this.errors.nombre = 'El nombre debe ser obligatorio.';
        ...
        if (!this.duracion || String(this.duracion).trim() === '') this.errors.duracion = 'La duración debe ser obligatoria.';
        else if (!Number.isInteger(dur) || dur < 1 || dur > 12) this.errors.duracion = 'La duración debe ser entre 1 y 12 meses.';
        else this.errors.duracion = '';

        // Valor
        let valStr = String(this.valor).trim();
        let valNum = Number(this.valor);
        if (!this.valor || valStr === '') this.errors.valor = 'El precio debe ser obligatorio.';
        ...
        if (!this.num_empl || String(this.num_empl).trim() === '') this.errors.num_empl = 'El número de empleados debe ser obligatorio.';
        else if (!Number.isInteger(emp) || emp < 1 || emp > 10000) this.errors.num_empl = 'El número de empleados debe ser entre 1 y 10,000.';
        ...
        this.features.forEach((f, i) => {
            ...
            if (fTrim && fTrim.length > 0) {
                ...
                else if (!featRegex.test(fTrim)) this.errors.features[i] = 'La característica debe contener solo símbolos permitidos.';
    ...
    $watch('nombre', () => validate());
    $watch('duracion', () => validate());
    $watch('valor', () => validate());
    $watch('num_empl', () => validate());
    $watch('descripcion', () => validate());
    $watch('stripe_price_id', () => validate());
    $watch('features', () => validate());
    validate();
">
